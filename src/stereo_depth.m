...
clear; clc; close all;
%% Connect to ROS (run roscore first or connect to Zynq ROS master)
% try
%     rosinit; 
% catch ME
%     if contains(ME.message, 'Global node already exists')
%         disp('[ROS] Already connected.');
%     else
%         rethrow(ME);
%     end
% end
% 
% pub = rospublisher('/object_position','geometry_msgs/Point');
% msg = rosmessage(pub);
...
%% Setup image input
frame = imread('/Users/apple/Documents/oneright.bmp');
...
%% Load inverse kinematics function path
addpath('/Users/apple/Documents/IK/ikiterative');  % replace with actual path if needed

%% Simulink prep (optional)
% Ensure Simulink is compiled once before entering loop
% bdclose('ModelWithInput');
load_system('/Users/apple/Documents/IK/ModelInput.slx');  % preload the Simulink model
...
if ishandle(hFig)
    ...
    if ~isempty(circularStats)
        ...
        if size(buffer, 1) > bufferSize
         ...
         offset = 0.05;
         target = [X_m, Y_m - offset, Z_m, X_m, Y_m + offset, Z_m];
         ikiterative(target);
        simOut = sim('/Users/apple/Documents/IK/ModelInput.slx');

        fprintf('[%s] Target (%.2f, %.2f, %.2f) passed to IK and simulated.\n', ...
           upper(object_type), X_m, Y_m, Z_m);
